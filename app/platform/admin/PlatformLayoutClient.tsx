...
const NAV = [
  { href: '/platform/admin', icon: LayoutDashboard, label: 'ภาพรวมระบบ' },
  { href: '/platform/admin/shops', icon: Store, label: 'ร้านพาร์ทเนอร์' },
  { href: '/platform/admin/discounts', icon: Ticket, label: 'โปรโมชั่น & แคมเปญ' },
  { href: '/platform/admin/crm', icon: Users, label: 'CRM ลูกค้าทั้งระบบ' },
  { href: '/platform/admin/reports', icon: ShieldAlert, label: 'รายงาน & เรื่องร้องเรียน' },
  { href: '/platform/admin/analytics', icon: BarChart3, label: 'Analytics & รายงาน' },
  { href: '/platform/admin/finance', icon: Wallet, label: 'การเงิน & ถอนเงิน' },
  { href: '/platform/admin/invitations', icon: Ticket, label: 'Invitation Codes' },
]
...
export default function PlatformLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  const currentNav = NAV.find(n => n.href === pathname || (n.href !== '/platform/admin' && pathname.startsWith(n.href)))
